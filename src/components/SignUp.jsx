...
const SignUp = () => {
    ...
    const signUp = async (data) => {
        setError('')
        try {
            const userData = await authServicesInstance.createAccount(data)

            if (userData) {
                const currentUser = await authServicesInstance.getCurrentUser()
                    ...
                    <div className="space-y-5">
                        <Input
                            ...
                            {...register('name', {
                                required: true,
                                validate: {
                                    matchPattern: (value) => /^[a-zA-Z\s]+$/.test(value) || "Name can only contain letters and spaces"
                                }
                            })}
                        />
                        <Input
